...
{
    Variable(string const name); // в конструкторе надо указать ее имя
...
FunctionCall::FunctionCall(string const &name, Expression const *arg)
...
string const &FunctionCall::name() const { return name_; }
Expression const *FunctionCall::arg() const { return arg_; }

Variable::Variable(string const name) : name_(name) {}
string const &Variable::name() const { return name_; }
...
int main()
{
    ...
    cout << "Original evaluate: " << callAbs->evaluate() << endl;
    cout << "Copied evaluate:   " << newExpr->evaluate() << endl;
